...
def optimize_mesh_normalmap(
    ...
    target_normalmap: torch.Tensor,               # Shape (1, H, W, 4), already permuted
    optimizer: torch.optim.Optimizer,
    iterations: int = 3000,
    silhouette_weight: float = 4.0,
    ...
    mesh.to(device)
    channel_weights = torch.tensor([1, 1, 1, silhouette_weight], dtype=torch.float32, device=device)

    criterion = nn.HuberLoss(reduction='mean')
    weighted_target = target_normalmap.float().to(device) * channel_weights
